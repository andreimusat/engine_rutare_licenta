...
public class ShortestPathRouter {
	...
	private void bendPaths() {
		for (int i = 0; i < orderedPaths.size(); i++) {
			PointList auxPoints = new PointList();
			...
			auxPoints.addPoint(path.points.getPoint(0));

			for (int j = 0; j < path.points.size() - 1; j++) {
				Point p1 = path.points.getPoint(j);
				Point p2 = path.points.getPoint(j + 1);

				if (p1.x == p2.x || p1.y == p2.y) {
					auxPoints.addPoint(p2);
				} else {
					auxPoints.addPoint(new Point(p1.x, p2.y));
					auxPoints.addPoint(p2);
				}
			}

			path.points.removeAllPoints();
			path.points.addAll(auxPoints);
		}
	}
