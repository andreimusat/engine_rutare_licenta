...
public abstract class AbstractLayoutAlgorithm implements LayoutAlgorithm,
	...
	class InternalComparator implements Comparator {
		...
		public int compare(Object o1, Object o2) {
			...
			return this.externalComparator.compare(
					internalNode1.getLayoutEntity(),
					internalNode2.getLayoutEntity());
		}
	...
	/**
	 * Queues up the given relationshp (if it isn't in the list) to be added to
	 * the algorithm.
	 * 
	 * @param relationship
	 */
	public void addRelationship(LayoutRelationship relationship) {
		if ((relationship != null)
				&& !relationshipsToAdd.contains(relationship)) {
	...
	/**
	 * Queues up the given relationship to be removed from the algorithm next
	 * time it runs.
	 * 
	 * @param relationship
	...
	public void removeRelationship(LayoutRelationship relationship) {
		if ((relationship != null)
				&& !relationshipsToRemove.contains(relationship)) {
	...
	public abstract void setLayoutArea(double x, double y, double width,
			double height);
	 ...
	 * @param x
	 *            The left side of the bounds in which the layout can place the
	 *            entities.
	 * @param y
	 *            The top side of the bounds in which the layout can place the
	 *            entities.
	 * @param width
	 *            The width of the bounds in which the layout can place the
	 *            entities.
	 * @param height
	 *            The height of the bounds in which the layout can place the
	 *            entities.
	 */
	abstract protected void applyLayoutInternal(
	...
	protected InternalNode[] updateEntities(InternalNode[] entities) {
		if ((entitiesToRemove.size() > 0) || (entitiesToAdd.size() > 0)) {
			...
			ArrayList updatedEntities = new ArrayList(internalNodes.length
					- entitiesToRemove.size() + entitiesToAdd.size());
			...
			// Add any new nodes
			LayoutEntity[] entitiesArray = new LayoutEntity[entitiesToAdd
					.size()];
			entitiesArray = (LayoutEntity[]) entitiesToAdd
					.toArray(entitiesArray);
			...
			internalNodes = (InternalNode[]) updatedEntities
					.toArray(internalNodes);
		}
	...
	/**
	 * Updates the given array of relationships checking if any need to be
	 * removed or added. Also updates the original array of relationships.
	 * 
	 * @param relationships
	 *            the current relationships
	 * @return the update relationships array
	 */
	protected InternalRelationship[] updateRelationships(
		...
		if ((relationshipsToRemove.size() > 0)
			...
			if (relationshipsToRemove.size() > 0) {
				for (Iterator iter = relationshipsToRemove.iterator(); iter
						.hasNext();) {
					LayoutRelationship relation = (LayoutRelationship) iter
							.next();
					if (relation.getLayoutInformation() != null) {
						internalRelsList
								.remove(relation.getLayoutInformation());
					}
				}
			}
			...
			ArrayList updatedRelationships = new ArrayList(
					internalRelationships.length - relationshipsToRemove.size()
							+ relationshipsToAdd.size());
			for (int i = 0; i < internalRelationships.length; i++) {
				InternalRelationship relation = internalRelationships[i];
				if (relationshipsToRemove.contains(relation
						.getLayoutRelationship())) {
					relationshipsToRemove.remove(relation
							.getLayoutRelationship());
				} else {
			...
			if (relationshipsToAdd.size() > 0) {
				LayoutRelationship[] relsArray = new LayoutRelationship[relationshipsToAdd
						.size()];
				relsArray = (LayoutRelationship[]) relationshipsToAdd
						.toArray(relsArray);
			...
			relationships = (InternalRelationship[]) internalRelsList
					.toArray(relationships);

			internalRelationships = new InternalRelationship[updatedRelationships
					.size()];
			internalRelationships = (InternalRelationship[]) updatedRelationships
					.toArray(internalRelationships);
		}
	 ...
	 * 
	 * @param dx
	 *            the amount to shift the entities in the x-direction
	 * @param dy
	 *            the amount to shift the entities in the y-direction
	 */
	/*
	...
	private void setupLayout(LayoutEntity[] entitiesToLayout,
	...
	// //setupLayout( entitiesToLayout, relationshipsToConsider, x, y, width,
	// height );
	 ...
	 */
	protected abstract void postLayoutAlgorithm(
			InternalNode[] entitiesToLayout,
	...
	public synchronized void applyLayout(final LayoutEntity[] entitiesToLayout,
		...
		if (asynchronous) {
			...
			Thread thread = new Thread(new Runnable() {
				...
				public void run() {
					setupLayout(entitiesToLayout, relationshipsToConsider, x,
							y, width, height);
					preLayoutAlgorithm(internalNodes, internalRelationships,
							internalX, internalY, internalWidth, internalHeight);
					fireProgressStarted(getTotalNumberOfLayoutSteps());

					applyLayoutInternal(internalNodes, internalRelationships,
							internalX, internalY, internalWidth, internalHeight);
					stop();
				}
	 ...
	 * @param relationshipsToConsider
	 */
	protected void updateBendPoints(
			InternalRelationship[] relationshipsToConsider) {
		for (int i = 0; i < relationshipsToConsider.length; i++) {
			InternalRelationship relationship = relationshipsToConsider[i];
			List<?> bendPoints = relationship.getBendPoints();
			if (bendPoints.size() > 0) {
				// We will assume that source/dest coordinates are for center of
				// node
				BendPoint[] externalBendPoints = new BendPoint[bendPoints
						.size() + 2];
				InternalNode sourceNode = relationship.getSource();
				externalBendPoints[0] = new BendPoint(
						sourceNode.getInternalX(), sourceNode.getInternalY());
				InternalNode destNode = relationship.getDestination();
				externalBendPoints[externalBendPoints.length - 1] = new BendPoint(
						destNode.getInternalX(), destNode.getInternalY());

				for (int j = 0; j < bendPoints.size(); j++) {
					BendPoint bp = (BendPoint) bendPoints.get(j);
					externalBendPoints[j + 1] = new BendPoint(bp.x, bp.y,
							bp.getIsControlPoint());
				}
				LayoutRelationship layoutRelationship = relationship
						.getLayoutRelationship();
				layoutRelationship.setBendPoints(externalBendPoints);
				layoutRelationship.toString();
			}
		}
	}
	...
	/**
	 * Creates a list of InternalNode objects from the list of LayoutEntity
	 * objects the user wants layed out. Sets the internal nodes' positions and
	 * sizes from the external entities.
	 */
	private InternalNode[] createInternalNodes(LayoutEntity[] nodes) {
		...
		for (int i = 0; i < nodes.length; i++) {
			...
			internalNode.setInternalLocation(externalNode.getXInLayout(),
					externalNode.getYInLayout());
	 ...
	 */
	private InternalRelationship[] createInternalRelationships(
			LayoutRelationship[] rels) {
		ArrayList listOfInternalRelationships = new ArrayList(rels.length);
		for (int i = 0; i < rels.length; i++) {
			LayoutRelationship relation = rels[i];
			InternalNode src = (InternalNode) relation.getSourceInLayout()
					.getLayoutInformation();
			InternalNode dest = (InternalNode) relation
					.getDestinationInLayout().getLayoutInformation();
			if ((src != null) && (dest != null)) {
				InternalRelationship internalRelationship = new InternalRelationship(
						relation, src, dest);
				listOfInternalRelationships.add(internalRelationship);
			} else {
				throw new RuntimeException(
						"Error creating internal relationship, one of the nodes is null: src="
								+ src + ", dest=" + dest);
			}
		}
	...
	private Object[] filterUnwantedObjects(LayoutItem[] objects) {
		...
		if (filter != null) {
			...
			// @tag bug.156266-ClassCast.fix : use reflection to create the
			// array.
			Object[] unfilteredObjs = (Object[]) java.lang.reflect.Array
					.newInstance(objects.getClass().getComponentType(),
	 ...
	 * @return
	 */
	protected DisplayIndependentPoint getLocalLocation(
			InternalNode[] entitiesToLayout, double x, double y,
		...
		DisplayIndependentRectangle layoutBounds = getLayoutBounds(
				entitiesToLayout, false);
		double localX = (x / screenWidth) * layoutBounds.width + layoutBounds.x;
		double localY = (y / screenHeight) * layoutBounds.height
				+ layoutBounds.y;
		return new DisplayIndependentPoint(localX, localY);
	}

	/**
	 * Find an appropriate size for the given nodes, then fit them into the
	 * given bounds. The relative locations of the nodes to each other must be
	 * preserved. Child classes should set flag reresizeEntitiesAfterLayout to
	 * false if they want to preserve node sizes.
	 */
	protected void defaultFitWithinBounds(InternalNode[] entitiesToLayout,
			DisplayIndependentRectangle realBounds) {
		defaultFitWithinBounds(entitiesToLayout, new InternalRelationship[0],
				realBounds);
	}
	...
	protected void defaultFitWithinBounds(InternalNode[] entitiesToLayout,
		...
		if (resizeEntitiesAfterLayout) {
			...
			convertPositionsToPercentage(entitiesToLayout, relationships,
					layoutBounds, false /* do not update size */);
		...
		// percentage)
		convertPositionsToPercentage(entitiesToLayout, relationships,
				layoutBounds, true /* update node size */);

		DisplayIndependentRectangle screenBounds = calcScreenBounds(realBounds,
				layoutBounds);
	...
	private DisplayIndependentRectangle calcScreenBounds(
		...
		if (resizeEntitiesAfterLayout) { // OK to alter aspect ratio
			double borderWidth = Math.min(realBounds.width, realBounds.height) / 10.0; // use
																						// 10%
																						// for
																						// the
																						// border
																						...
																						// each
																						// side
			return new DisplayIndependentRectangle(realBounds.x + borderWidth
					/ 2.0, realBounds.y + borderWidth / 2.0, realBounds.width
					- borderWidth, realBounds.height - borderWidth);
			...
			double adjustedX = realBounds.x
					+ (realBounds.width - adjustedWidth) / 2.0;
			double adjustedY = realBounds.y
					+ (realBounds.height - adjustedHeight) / 2.0;
			double borderWidth = Math.min(adjustedWidth, adjustedHeight) / 10.0; // use
																					// 10%
																					// for
																					// the
																					// border
																					...
																					// each
																					// side
			return new DisplayIndependentRectangle(adjustedX + borderWidth
					/ 2.0, adjustedY + borderWidth / 2.0, adjustedWidth
					- borderWidth, adjustedHeight - borderWidth);
		}
	}

	/**
	 * Find and set the node size - shift the nodes to the right and down to
	 * make room for the width and height.
	 * 
	 * @param entitiesToLayout
	 * @param relationships
	 */
	...
	private void convertPositionsToPercentage(InternalNode[] entitiesToLayout,
		...
		for (int i = 0; i < entitiesToLayout.length; i++) {
			InternalNode node = entitiesToLayout[i];
			DisplayIndependentPoint location = node.getInternalLocation()
					.convertToPercent(layoutBounds);
		...
		for (int i = 0; i < relationships.length; i++) {
			...
			for (int j = 0; j < rel.getBendPoints().size(); j++) {
				...
				DisplayIndependentPoint toPercent = bp
						.convertToPercent(layoutBounds);
				bp.setX(toPercent.x);
				bp.setY(toPercent.y);
			}
	...
	private void convertPositionsToCoords(InternalNode[] entitiesToLayout,
		...
		for (int i = 0; i < entitiesToLayout.length; i++) {
			...
			DisplayIndependentPoint location = node.getInternalLocation()
					.convertFromPercent(screenBounds);
			node.setInternalLocation(location.x - width / 2, location.y
					- height / 2);
			if (resizeEntitiesAfterLayout) {
		...
		for (int i = 0; i < relationships.length; i++) {
			...
			for (int j = 0; j < rel.getBendPoints().size(); j++) {
				BendPoint bp = (BendPoint) rel.getBendPoints().get(j);
				DisplayIndependentPoint fromPercent = bp
						.convertFromPercent(screenBounds);
	 ...
	 * @param width
	 */
	private void adjustNodeSizeAndPos(InternalNode node, double height,
			double width) {
		...
		if (widthToHeightRatio <= 1.0 && widthUsingHeight <= width) {
			...
			node.setInternalSize(
					Math.max(height * widthToHeightRatio, MIN_ENTITY_SIZE),
					Math.max(height, MIN_ENTITY_SIZE));
			node.setInternalLocation(node.getInternalX() + leftOut / 2,
					node.getInternalY());
			...
			node.setInternalLocation(node.getInternalX(), node.getInternalY()
					+ leftOut / 2);
		}
	...
	/**
	 * Returns the maximum possible node size as a percentage of the width or
	 * height in current coord system.
	 */
	...
	protected DisplayIndependentRectangle getLayoutBounds(
		...
		for (int i = 0; i < entitiesToLayout.length; i++) {
			...
			if (includeNodeSize) {
				leftSide = Math.min(
						entity.getInternalX() - entity.getInternalWidth() / 2,
						leftSide);
				topSide = Math.min(
						entity.getInternalY() - entity.getInternalHeight() / 2,
						topSide);
				rightSide = Math.max(
						entity.getInternalX() + entity.getInternalWidth() / 2,
						rightSide);
	 ...
	 * 
	 */
	private DisplayIndependentDimension getMinimumDistance(
			InternalNode[] entitiesToLayout) {
		DisplayIndependentDimension horAndVertdistance = new DisplayIndependentDimension(
				Double.MAX_VALUE, Double.MAX_VALUE);
		double minDistance = Double.MAX_VALUE; // the minimum distance between
												// all the nodes
		// TODO: Very Slow!
		for (int i = 0; i < entitiesToLayout.length; i++) {
	...
	/**
	 * Returns the width to height ratio this layout will use to set the size of
	 * the entities.
	 */
	 ...
	 * To relieve some of the mystery, the layout algorithm will notify each
	...
	/**
	 * Removes the given progress listener, preventing it from receiving any
	 * more updates.
	 */
		...
		for (int i = 0; i < nodes.length; i++) {
			...
			if (!node.hasPreferredLocation()) {
				...
				if ((layout_styles & LayoutStyles.NO_LAYOUT_NODE_RESIZING) != 1) {
					...
					node.setSize(node.getInternalWidth(),
							node.getInternalHeight());
				}
			}
		}
	}
	...
	protected void fireProgressStarted(int totalNumberOfSteps) {
		...
		for (int i = 0; i < progressListeners.size(); i++) {
			ProgressListener listener = (ProgressListener) progressListeners
					.get(i);
	...
	protected void fireProgressEnded(int totalNumberOfSteps) {
		ProgressEvent event = new ProgressEvent(totalNumberOfSteps,
				totalNumberOfSteps);
		for (int i = 0; i < progressListeners.size(); i++) {
			ProgressListener listener = (ProgressListener) progressListeners
					.get(i);
	...
	/**
	 * Fires an event to notify all of the registered ProgressListeners that
	 * another step has been completed in the algorithm.
	 * 
	 * @param currentStep
	...
	protected void fireProgressEvent(int currentStep, int totalNumberOfSteps) {
		...
		if (now.after(lastProgressEventFired)
				|| currentStep == totalNumberOfSteps) {
			ProgressEvent event = new ProgressEvent(currentStep,
					totalNumberOfSteps);
