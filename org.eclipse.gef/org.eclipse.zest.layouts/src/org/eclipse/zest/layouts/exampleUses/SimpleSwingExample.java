...
import org.eclipse.zest.layouts.algorithms.OrthogonalLayoutAlgorithm;
...
public class SimpleSwingExample {
	...
	// private static final Color NODE_ADJACENT_COLOR = new Color (255, 200,
	// 125);
	...
	// private static final Color BORDER_ADJACENT_COLOR = new Color (255, 128,
	// 0);
	...
	public static TreeLayoutAlgorithm TREE_VERT = new TreeLayoutAlgorithm(
			LayoutStyles.NONE);
	public static HorizontalTreeLayoutAlgorithm TREE_HORIZ = new HorizontalTreeLayoutAlgorithm(
			LayoutStyles.NONE);
	...
	public static GridLayoutAlgorithm GRID = new GridLayoutAlgorithm(
			LayoutStyles.NONE);
	public static HorizontalLayoutAlgorithm HORIZ = new HorizontalLayoutAlgorithm(
			LayoutStyles.NONE);
	public static VerticalLayoutAlgorithm VERT = new VerticalLayoutAlgorithm(
			LayoutStyles.NONE);
	public static OrthogonalLayoutAlgorithm ORTO = new OrthogonalLayoutAlgorithm(
			LayoutStyles.NONE);
	...
	private static final Shape ARROW_SHAPE = new Polygon(new int[] {
			-ARROW_HALF_HEIGHT, ARROW_HALF_HEIGHT, -ARROW_HALF_HEIGHT },
	...
	protected String currentNodeShape = DEFAULT_NODE_SHAPE; // e.g., oval,
															// rectangle
	...
	public void start() {
		...
		mainFrame.setLocation(
				(int) (screenSize.getWidth() - INITIAL_PANEL_WIDTH) / 2,
		...
		try {
			SwingUtilities.invokeAndWait(new Runnable() {
				...
				public void run() {
					...
					TREE_HORIZ = new HorizontalTreeLayoutAlgorithm(
							LayoutStyles.NONE);
					...
					ORTO = new OrthogonalLayoutAlgorithm(LayoutStyles.NONE);
					...
					addAlgorithm(ORTO, "Orto", false);

					for (int i = 0; i < algorithms.size(); i++) {
						final LayoutAlgorithm algorithm = (LayoutAlgorithm) algorithms
								.get(i);
						final String algorithmName = (String) algorithmNames
								.get(i);
						...
						algorithmButton.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent e) {
								...
								algorithm
										.setEntityAspectRatio((double) mainPanel
												.getWidth()
												/ (double) mainPanel
														.getHeight());
	...
	private void stop() {
		if (currentLayoutAlgorithm != null
				&& currentLayoutAlgorithm.isRunning()) {
	...
	protected void performLayout() {
		...
		ProgressListener progressListener = new ProgressListener() {
			public void progressUpdated(final ProgressEvent e) {
				...
				// }
				lblProgress
						.setText("Progress: " + e.getStepsCompleted() + " of "
								+ e.getTotalNumberOfSteps() + " completed ...");
				lblProgress.paintImmediately(0, 0, lblProgress.getWidth(),
						lblProgress.getHeight());
			}
		...
		try {
			final LayoutEntity[] layoutEntities = new LayoutEntity[entities
					.size()];
			entities.toArray(layoutEntities);
			final LayoutRelationship[] layoutRelationships = new LayoutRelationship[relationships
					.size()];
	...
	private void createMainPanel() {
		...
		mainPanel.setPreferredSize(new Dimension(INITIAL_PANEL_WIDTH,
		...
		mainPanel.addMouseListener(new MouseAdapter() {
			public void mousePressed(MouseEvent e) {
				...
				if (selectedEntity != null) {
					...
					selectedEntityPositionAtMouseDown = new Point(
							(int) selectedEntity.getX(), (int) selectedEntity
									.getY());
				} else {
					mouseDownPoint = null;
	 ...
	 * 
	 * @param maxLevels
	 *            Max number of levels wanted in tree
	 * @param maxChildren
	 *            Max number of children for each node in the tree
	 * @param randomNumChildren
	 *            Whether or not to pick random number of levels (from 1 to
	 *            maxLevels) and random number of children (from 1 to
	 *            maxChildren)
	 */
	private void createTreeGraph(int minChildren, int maxChildren,
			int minLevels, int maxLevels, boolean randomNumChildren,
		...
		createTreeGraphRecursive(currentParent, minChildren, maxChildren,
				minLevels, maxLevels, 1, randomNumChildren, randomLevels,
				addNonTreeRels);
	}

	private void createTreeGraphRecursive(LayoutEntity currentParentNode,
			int minChildren, int maxChildren, int minLevel, int maxLevel,
			int level, boolean randomNumChildren, boolean randomLevels,
		...
		int numChildren = randomNumChildren ? Math.max(minChildren,
				(int) (Math.random() * maxChildren + 1)) : maxChildren;
		for (int i = 0; i < numChildren; i++) {
			...
			if (addNonTreeRels && entities.size() % 5 == 0) {
				...
				LayoutRelationship rel = new SimpleRelationship(
						(LayoutEntity) entities.get(index), newNode, false);
				relationships.add(rel);
			}
			LayoutRelationship rel = new SimpleRelationship(currentParentNode,
					newNode, false);
			relationships.add(rel);
			createTreeGraphRecursive(newNode, minChildren, maxChildren,
					minLevel, maxLevel, level + 1, randomNumChildren,
					randomLevels, addNonTreeRels);
		}
	}

	/**
	 * Call this from createGraph in place of createTreeGraph this for debugging
	 * and testing.
	 */
	/*
	 * private void createCustomGraph() { LayoutEntity A =
	 * createSimpleNode("1"); LayoutEntity B = createSimpleNode("10");
	 * LayoutEntity _1 = createSimpleNode("100"); entities.add(A);
	 * entities.add(B); entities.add(_1); relationships.add(new
	 * SimpleRelationship (A, B, false)); relationships.add(new
	 * SimpleRelationship (A, _1, false)); relationships.add(new
	 * SimpleRelationship (_1, A, false)); }
	 */
	...
	private void placeRandomly() {
		for (Iterator iter = entities.iterator(); iter.hasNext();) {
			...
			double y = Math.random() * INITIAL_PANEL_HEIGHT
					- INITIAL_NODE_HEIGHT;
	...
	private void updateGUI() {
		updateGUICount++;
		if (updateGUICount > 0) {
			mainPanel.paintImmediately(0, 0, mainPanel.getWidth(),
					mainPanel.getHeight());
		}
	}

	private static Point2D.Double getEllipseIntersectionPoint(double theta,
	...
	/**
	 * A JPanel that provides entity and relationship rendering Instead of
	 * letting Swing paint all the JPanels for us, we will just do our own
	 * painting here
	 */
	private class MainPanel extends JPanel {
		...
		private void paintEntity(SimpleNode entity, Graphics g) {
			boolean isSelected = selectedEntity != null
			...
			if (currentNodeShape.equals("rectangle")) {
				g.fillRect((int) entity.getX(), (int) entity.getY(),
						(int) entity.getWidth(), (int) entity.getHeight());
			} else { // default
				g.fillOval((int) entity.getX(), (int) entity.getY(),
						(int) entity.getWidth(), (int) entity.getHeight());
			}
			g.setColor(isSelected ? BORDER_SELECTED_COLOR : BORDER_NORMAL_COLOR);
			String name = entity.toString();
			Rectangle2D nameBounds = g.getFontMetrics()
					.getStringBounds(name, g);
			g.drawString(
					name,
					(int) (entity.getX() + entity.getWidth() / 2.0 - nameBounds
							.getWidth() / 2.0),
					(int) (entity.getY() + entity.getHeight() / 2.0 + nameBounds
							.getHeight() / 2.0));// - nameBounds.getHeight() -
													// nameBounds.getY()));
			if (g instanceof Graphics2D) {
				((Graphics2D) g).setStroke(isSelected ? BORDER_SELECTED_STROKE
			...
			if (currentNodeShape.equals("rectangle")) {
				...
				g.drawOval((int) entity.getX(), (int) entity.getY(),
						(int) entity.getWidth(), (int) entity.getHeight());
			}
		}
		...
		/**
		 * Draw a line from the edge of the src node to the edge of the
		 * destination node
		 */
		private void drawRelationship(SimpleNode src, SimpleNode dest,
				double theta, double srcX, double srcY, double destX,
				double destY, Graphics g) {

			double reverseTheta = theta > 0.0d ? theta - Math.PI : theta
					+ Math.PI;

			Point2D.Double srcIntersectionP = getEllipseIntersectionPoint(
					theta, src.getWidth(), src.getHeight());

			Point2D.Double destIntersectionP = getEllipseIntersectionPoint(
					reverseTheta, dest.getWidth(), dest.getHeight());

			drawRelationship(srcX + srcIntersectionP.getX(), srcY
					+ srcIntersectionP.getY(),
					destX + destIntersectionP.getX(),
					destY + destIntersectionP.getY(), g);
		}

		/**
		 * Draw a line from specified source to specified destination
		 */
		private void drawRelationship(double srcX, double srcY, double destX,
				double destY, Graphics g) {
			g.setColor(RELATIONSHIP_NORMAL_COLOR);
			g.drawLine((int) srcX, (int) srcY, (int) destX, (int) destY);
		}

		private void drawArrow(double theta, double srcX, double srcY,
				double dx, double dy, Graphics g) {
		...
		private void drawCurvedRelationship(double srcX, double srcY,
				double control1X, double control1Y, double control2X,
				double control2Y, double destX, double destY, Graphics g) {
			GeneralPath shape = new GeneralPath();
			shape.moveTo((float) srcX, (float) srcY);
			shape.curveTo((float) control1X, (float) control1Y,
					(float) control2X, (float) control2Y, (float) destX,
					(float) destY);
			g.setColor(RELATIONSHIP_NORMAL_COLOR);
		 ...
		 * drawn. Note that this assumes the first and last bendpoints are
		 * actually the source node and destination node centre points.
		 * 
		 * @param relationship
		 * @param bendNodes
		 * @param bendEdges
		...
		private void drawBendPoints(LayoutRelationship rel, Graphics g) {
			...
			LayoutBendPoint[] bendPoints = ((SimpleRelationship) rel)
					.getBendPoints();

			srcX = bendPoints[1].getX();
			srcY = bendPoints[1].getY();
			int bpNum = 2;
			while (bpNum < bendPoints.length - 1) { // ignore first and last
													// bendpoints (src and dest)
				...
				if (bp.getIsControlPoint()) {
					if (bendPoints[bpNum + 1].getIsControlPoint()) {
						destEntity = new SimpleNode(DUMMY_TITLE,
								bendPoints[bpNum + 2].getX(),
								bendPoints[bpNum + 2].getY(), 0.01, 0.01);
						drawCurvedRelationship(srcX, srcY, bp.getX(),
								bp.getY(), bendPoints[bpNum + 1].getX(),
								bendPoints[bpNum + 1].getY(),
								bendPoints[bpNum + 2].getX(),
								bendPoints[bpNum + 2].getY(), g);
						bpNum += 4;
					} else {
						destEntity = new SimpleNode(DUMMY_TITLE, bp.getX(),
								bp.getY(), 0.01, 0.01);
					}
				} else {
